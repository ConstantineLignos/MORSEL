...
public class CorpusLoaderTest extends TestCase {
	String wordListPath = "data/test/testwordlist.eng";
	
	public void testloadWordlist() {
		Lexicon lex = CorpusLoader.loadWordlist(wordListPath);
		assertNotNull(lex.getWord("cat"));
		assertEquals(200, lex.getWord("cat").getCount());
	}
	
	public void testparseWordlistEntry() {
		assertEquals(new Word("dog", 100, true), 
				CorpusLoader.parseWordlistEntry("100 dog"));
	}
