/*
 * Copyright (C) 2009-2019 Constantine Lignos
 *
...
public class Compounding {
   ...
   */
  public static int breakCompounds(
      Lexicon lex,
      WordSet set,
      Collection<Transform> learnedTransforms,
   ...
   * @return the best hypothesis for breaking the word, null if the word should not be broken
   */
  protected static Hypothesis breakWord(
    ...
    while (!currHyps.isEmpty()) {
      ...
      for (i = 0; i < BEAM_SIZE; i++) {
        ...
        List<Hypothesis> results = hyp.extendAll(lex, filler, transInf, doubling);
        ...
        for (Hypothesis result : results) {
          ...
          if (result.isComplete()
              && !(result.words.size() == 1 && result.words.get(0).getDerivation() == null)) {
    ...
    return completeHyps.isEmpty() ? null : Hypothesis.pickHypothesis(completeHyps, word);
  }

  protected static List<Word> getPrefixes(
      String word,
      Lexicon lex,
      boolean allowFull,
      Filler fillers,
    ...
    if (fillers != null) {
      for (int i = MIN_COMPOUND_LENGTH; i <= max; i++) {
        ...
        if (baseWord.getSet() == WordSet.UNMODELED) {
          continue;
        }

        // Then try each prefix with a filler suffix- these are derived
        // forms
        for (FillerResult result : fillers.getFilledSuffixes(prefix, word, baseWord, doubling)) {
          ...
          if (TRANSFORM_RELATIONS
              && !transInf.isGoodRelation(result.baseWord.getDerivation(), result.derivation)) {
            continue;
          }
  ...
  /**
   * Perform simplex word analysis. This is an experimental feature which tries to segment words
   * without using base/derived pairs, just by splitting a word into morphemes if it remains
   * unmodeled at the end of learning.
   *
   ...
   */
  public static int analyzeSimplexWords(
      Lexicon lex,
      WordSet set,
      Collection<Transform> learnedTransforms,
      boolean doubling,
  ...
  private static AnalysisResult analyzeWord(
      Word word, Filler filler, Lexicon lex, TransformInference transInf, boolean doubling) {
    ...
    while (!currResults.isEmpty()) {
      ...
      for (i = 0; i < BEAM_SIZE; i++) {
        ...
        List<AnalysisResult> results =
            hyp.extendAll(lex, filler, transInf, doubling, word.getText());
        // Check each extended one to see if it's complete
  ...
  /**
   * The FillerResult class is a simple structure to represent the result of adding filler to a
   * word.
   */
  protected static class FillerResult {
     ...
     * @param derivation the deriving transform to connect the base and derived forms
     */
    public FillerResult(String derivedText, Word baseWord, Transform derivation) {
  ...
  /**
   * The Filler class takes learned transforms and can apply them as needed to word prefixes to form
  ...
  protected static class Filler {
    ...
    public static String makeFillerDerivedFromPrefix(
        String fullWord, String prefixWord, boolean doubling, Affix affix1, Affix affix2) {
      String derived;
      // Try possible derived forms using each transform
      // Check for affix1
      if (!Affix.hasAffix(prefixWord, affix1)) return null;

      String stem = Transform.makeStem(prefixWord, affix1);
      derived = Transform.makeDerived(stem, affix2, false, false);

      // Check derived, break if it's good
      if (fullWord.startsWith(derived)) return derived;

      // Try doubled and undoubled forms if needed and do the same
      if (doubling && affix1.isNull()) {
        // Doubled
        derived = Transform.makeDerived(stem, affix2, true, false);
        if (fullWord.startsWith(derived)) return derived;

        // Undoubled, make sure it's not the same as the stem
        derived = Transform.makeDerived(stem, affix2, false, true);
        if (fullWord.startsWith(derived) && !derived.equals(stem)) return derived;
      }
    ...
    /**
     * Return a list of FillerResults representing all legal combinations of the prefix and fillers.
     *
  ...
  private static class AnalysisResult {
     ...
     */
    public List<AnalysisResult> extendAll(
        Lexicon lex,
        Filler filler,
        TransformInference transInf,
        boolean doubling,
      ...
      for (Transform transform : filler.suffixes) {
        ...
        String derived =
            Filler.makeFillerDerivedFromPrefix(
                fullWord, text, doubling, transform.getAffix1(), transform.getAffix2());
